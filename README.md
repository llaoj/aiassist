...
- 🎯 **上下文感知**：自动关联命令执行结果，支持连续对话，最多10层递归分析
- 📊 **管道分析**：直接分析命令输出流，支持13,000行nginx日志，如 `tail -f access.log | aiassist`
- 🛡️ **安全控制**：查询命令（绿色）和修改命令（红色）差异化展示，选择列表确认，修改命令二次确认
- 🌍 **多语言支持**：中文/英文界面
...
**工作流程：**
1. 管道前的命令输出作为输入（最多1.6MB，约13,000行nginx日志）
2. AI 自动分析数据，识别异常
3. 给出诊断结论和解决方案
4. 管道模式为非交互式，仅显示分析结果后退出
...
每个 LLM Provider 都需要配置 API Key 才能使用。API Key 是访问模型服务的凭证，请妥善保管。

**模型状态说明：**
- `✓ Enabled` - 模型已启用且可用
- `✗ Disabled` - 模型已被用户禁用
- `✗ Unavailable` - **模型暂时不可用**（额度用尽或达到限制次数）

当模型返回 HTTP 429 状态码时，表示该模型的 API 调用次数已达到限制或额度已用尽，此时模型会被标记为 `Unavailable`。系统会自动切换到下一个可用模型继续服务。
...
任何实现 OpenAI API 标准的服务都可以配置, 比如 openrouter(https://openrouter.ai/):
