...
**本地模式：**
- 使用 `aiassist config` 命令修改
- 或直接编辑 `~/.aiassist/config.yaml`
...
- ✅ **允许本地修改** - 可以使用 `aiassist config` 命令

### 配置保存

**配置中心模式（`consul.enabled: true`）：**
- ❌ **禁止通过命令修改** - `aiassist config` 等命令会被拒绝
- ✅ **统一在 Consul 修改** - 通过 Consul UI 或 `consul kv put` 命令修改
- 📝 **只读模式** - 本地命令只能查看配置，不能修改

**本地模式：**
- ✅ 可以通过 `aiassist config` 命令修改
- ✅ 保存到本地文件 `~/.aiassist/config.yaml`
...
A: 不能使用 `aiassist config` 命令。需要在 Consul UI 或通过 `consul kv put` 命令修改配置。
...
**Q: 为什么执行 `aiassist config` 提示错误？**

A: 配置中心模式下不允许通过命令修改配置。检查本地配置文件是否启用了 Consul（`consul.enabled: true`）。如需修改，请在 Consul 中操作。
